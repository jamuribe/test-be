...
A distributed, dependency-aware background worker system built with TypeScript, Node.js, Express, and TypeORM. This application parses multi-step analytical pipelines from YAML definitions, manages complex sequential task execution, runs heavy geospatial calculations, and aggregates results dynamically.

Architecture Overview
The system is separated into three decoupled execution layers:

API Router (Express): Ingests incoming client GeoJSON data, processes the YAML workflow blueprint, maps relationships, and queues tasks.

Task Worker (Polling Loop): A continuous, strict FIFO background polling service that monitors the database queue, evaluating execution guard rails and explicit task dependencies before releasing a job.

Task Runner (Job Engine): Executes specific concrete business logic (e.g., computing polygon surface areas) and stamps aggregated data reports directly onto finished workflows.
